...
function ProfilesPrelude({chart}:{chart:Chart}){const keys=profileOrder(chart);return <div className="profilesPrelude"><p className="eyebrow">UNA PERSONA, VARIAS SITUACIONES</p><h2>Cada parte de tu carta describe cómo actúas en una situación distinta.</h2><div className={`profileCurtain profiles-${keys.length}`} aria-hidden="true">{keys.map((key,index)=><i key={key}><span>0{index+1}</span><b>{pillarMeta[key].eyebrow}</b></i>)}</div></div>}
