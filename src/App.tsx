...
function BirthForm({onSubmit,onBack}:{onSubmit:(x:BirthInput)=>void;onBack?:()=>void}){
  ...
  return <main className="shell formPage">
    ...
    <form className="birthForm" onSubmit={submit}>
      ...
      <div className="fieldRow"><label>Fecha de nacimiento<input type="date" value={date} onChange={e=>setDate(e.target.value)} min="1900-01-01" max="2050-12-31" required/></label><label>Hora de nacimiento <span>{timeUnknown?'Hora abierta':''}</span><input type="time" value={time} onChange={e=>setTime(e.target.value)} disabled={timeUnknown} required={!timeUnknown}/><button type="button" className="unknownTimeToggle" aria-pressed={timeUnknown} onClick={()=>{setTimeUnknown(value=>!value);setTime('')}}>{timeUnknown?'Ingresar una hora':'No conozco mi hora'}</button></label></div>
