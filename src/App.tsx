...
const ACTIVE_STORE='mi-mapa.active.v1'
const STORY_STORE='mi-mapa.story-progress.v1'
...
const elementPortrait:Record<ElementKey,string>={
  wood:'Cuando ves una posibilidad, empiezas pronto y ajustas el plan mientras avanzas.',
  fire:'Cuando algo te entusiasma, propones ideas, participas y animas a otras personas a empezar.',
  earth:'Cuando te comprometes, organizas las tareas y cumples con lo acordado.',
  metal:'Ves rápido qué sobra, qué falta y dónde conviene poner un límite.',
  water:'Antes de actuar, observas, relacionas detalles y consideras varias opciones.',
}
...
const actionMeta:Record<TenGodKey,{name:string;copy:string}>={
  ...
  qi_sha:{name:'Responder al reto',copy:'Cuando algo se complica, sueles responder con más decisión y claridad.'},
  zheng_guan:{name:'Ordenar',copy:'Con responsabilidades y reglas claras, te resulta mucho más fácil avanzar.'},
  pian_yin:{name:'Recordar detalles',copy:'Guardas detalles pequeños y los recuerdas cuando necesitas resolver un problema.'},
  zheng_yin:{name:'Comprender primero',copy:'Te sirve entender bien cómo funciona algo antes de tomar una decisión.'},
}
...
function activeItem(items:SavedMap[]){
  const requested=localStorage.getItem(ACTIVE_STORE)
  return items.find(item=>item.id===requested)||items[0]||null
}
function rememberActive(input:BirthInput,items:SavedMap[]){
  const item=items.find(entry=>sameBirth(entry.input,input))
  if(item)localStorage.setItem(ACTIVE_STORE,item.id)
}
function readStoryProgress(input:BirthInput|null,items:SavedMap[]){
  if(!input)return 0
  const item=items.find(entry=>sameBirth(entry.input,input))
  if(!item)return 0
  try{
    const progress=JSON.parse(localStorage.getItem(STORY_STORE)||'{}') as Record<string,number>
    return Number.isInteger(progress[item.id])?Math.max(0,progress[item.id]):0
  }catch{return 0}
}
function writeStoryProgress(input:BirthInput,items:SavedMap[],step:number){
  const item=items.find(entry=>sameBirth(entry.input,input))
  if(!item)return
  try{
    const progress=JSON.parse(localStorage.getItem(STORY_STORE)||'{}') as Record<string,number>
    progress[item.id]=step;localStorage.setItem(STORY_STORE,JSON.stringify(progress))
  }catch{localStorage.setItem(STORY_STORE,JSON.stringify({[item.id]:step}))}
}
function clearStoryProgress(input:BirthInput,items:SavedMap[]){
  const item=items.find(entry=>sameBirth(entry.input,input))
  if(!item)return
  try{const progress=JSON.parse(localStorage.getItem(STORY_STORE)||'{}') as Record<string,number>;delete progress[item.id];localStorage.setItem(STORY_STORE,JSON.stringify(progress))}catch{/* El siguiente guardado crea un registro limpio. */}
}
...
export default function App(){
  ...
  const initialRoute=useMemo(()=>routeFromLocation(location.pathname,location.hash),[])
  ...
  useEffect(()=>{
    const syncRoute=()=>{
      const route=routeFromLocation(location.pathname,location.hash)
      setReadingSection(route.section)
      if(routeNeedsChart(route)&&!active){pendingRoute.current=route;setView('form')}
      else if(route.view==='home')setView(library.length?'home':'form')
      else setView(route.view)
      if(route.view==='stories')setStoryStep(readStoryProgress(active,library))
      window.scrollTo(0,0)
    }
  ...
  const save=(input:BirthInput,completed=false)=>{
    ...
    setLibrary(next);persist(next);localStorage.setItem(ACTIVE_STORE,entry.id);return entry
  }
  const goRoute=(route:AppRoute,input=active,replace=false)=>{
    if(input){setActive(input);rememberActive(input,library)}
    setView(route.view);setReadingSection(route.section);window.scrollTo(0,0)
    const url=new URL(window.location.href)
    url.pathname=pathForRoute(route);url.hash='';url.searchParams.delete('c');url.searchParams.delete('vista')
    if(route.view!=='today'&&route.view!=='calendar')url.searchParams.delete('fecha')
    if(route.view!=='today')url.searchParams.delete('origen')
    if(route.view!=='month')url.searchParams.delete('periodo')
    if(route.view!=='calendar'){url.searchParams.delete('modo');url.searchParams.delete('actividad');url.searchParams.delete('anio')}
    history[replace?'replaceState':'pushState']({},'',url)
  }
  const go=(target:View,input=active,replace=false)=>goRoute({view:target},input,replace)
  const start=(input:BirthInput)=>{save(input);setStoryStep(0);const requested=pendingRoute.current;pendingRoute.current=null;requested?goRoute(requested,input,true):go('stories',input)}
  const open=(item:SavedMap,target:View='reading')=>{setStoryStep(target==='stories'?readStoryProgress(item.input,library):0);go(target,item.input)}
  const finish=()=>{if(active){save(active,true);clearStoryProgress(active,library)}go('reading',active,true)}
  const updateActive=(changes:Partial<BirthInput>)=>{if(!active)return;const next={...active,...changes};save(next);go(view,next,true)}
  const openReadingSection=(section:ReadingSection)=>goRoute({view:'reading',section},active)
...
function Studio(){return <SocialStudio/>}
...
function BirthForm({onSubmit,onBack}:{onSubmit:(x:BirthInput)=>void;onBack?:()=>void}){
  ...
  return <main className="shell formPage">
    ...
    <section className="formIntro"><p className="eyebrow">TU MOMENTO DE NACER</p><h1>Empecemos<br/><em>por ti.</em></h1><p>Con estos datos calculamos el año, mes y día de tu carta. Cuando conoces tu hora, también calculamos el cuarto perfil. Todo se procesa dentro de tu dispositivo.</p></section>
...
function LoadingStory(){return <StoryCenter kicker="UNA TRADICIÓN DE MÁS DE MIL AÑOS"><div className="loader"><i/><i/><i/><span>十</span></div><h2>Ordenando tu mapa</h2><p>Estamos calculando el año, mes, día y hora de tu carta.</p></StoryCenter>}
...
function ProfilesPrelude({chart}:{chart:Chart}){const keys=profileOrder(chart);return <div className="profilesPrelude"><p className="eyebrow">UNA PERSONA, VARIAS SITUACIONES</p><h2>Cada perfil de tu carta describe cómo actúas en una situación distinta.</h2><div className={`profileCurtain profiles-${keys.length}`} aria-hidden="true">{keys.map((key,index)=><i key={key}><span>0{index+1}</span><b>{pillarMeta[key].eyebrow}</b></i>)}</div></div>}
...
function ElementStory({element,label,strongest=false}:{element:ElementKey;label:string;count:number;strongest?:boolean}){const meta=elementMeta[element];return <StoryCenter kicker={label}><ElementMark element={element}/><h1>{meta.label}</h1><p>{strongest?`${meta.sentence} Es uno de los recursos que usas con mayor frecuencia.`:`${meta.sentence} Suele requerir más práctica.`}</p></StoryCenter>}
function ElementsStory({chart}:{chart:Chart}){return <div className="elementsStory"><p className="eyebrow">TUS CINCO RECURSOS</p><h2>Esta es la mezcla que usas cada día.</h2><ElementRow chart={chart}/><p>La altura muestra cuáles usas con mayor frecuencia y cuáles requieren más práctica.</p><ShareActions kind="elements" chart={chart} shareLabel="Compartir mi gráfica"/></div>}

function topActions(chart:Chart){return (Object.entries(chart.tenGods) as [TenGodKey,number][]).sort((a,b)=>b[1]-a[1]).slice(0,3)}
function ActionsStory({chart}:{chart:Chart}){const labels=['La usas con mayor frecuencia','También la usas seguido','Otra respuesta frecuente'];return <div className="actionsStory"><p className="eyebrow">TUS FORMAS DE ACTUAR</p><h2>Tres respuestas que aparecen cuando algo importa.</h2><div className="actionList">{topActions(chart).map(([key],i)=><article key={key}><span aria-hidden="true">→</span><div><small>{labels[i]}</small><b>{actionMeta[key].name}</b><p>{shortStoryCopy(actionMeta[key].copy,100)}</p></div></article>)}</div><ShareActions kind="actions" chart={chart} shareLabel="Compartir mis formas de actuar"/></div>}
function InteractionsStory({chart}:{chart:Chart}){const data=chart.interactions[0]?interactionReading(chart.interactions[0]):{title:`Tus ${profileCountLabel(chart)} perfiles suelen actuar por separado`,body:'Cada perfil aparece en situaciones distintas.'};return <div className="interactionStory"><p className="eyebrow">CÓMO SE RELACIONAN TUS PERFILES</p><h2>{data.title}</h2><p>{shortStoryCopy(data.body,150)}</p><div className="orbit" aria-hidden="true"><i/><i/><span>十</span></div></div>}
function FinalStory({chart}:{chart:Chart}){const identity=identityMeta[chart.dayMaster.stem],meta=elementMeta[chart.dayMaster.element],polarity=chart.dayMaster.polarity==='yin'?'Yin':'Yang';return <div className="finalStory"><p className="eyebrow">MI MAPA EN BREVE</p><div className="finalCard"><Glyph stem={chart.dayMaster.stem} size={74}/><small>TU CENTRO</small><h2>{meta.label} {polarity}</h2><p>{identity.headline}</p><div className="finalBrief"><span>Una forma de volver a ti</span><b>{identity.name}</b></div></div><ShareActions kind="summary" chart={chart} shareLabel="Compartir mi mapa en breve"/></div>}

function saveLater(input:BirthInput,library:SavedMap[],step:number,notify:(x:string)=>void){
  writeStoryProgress(input,library,step)
  const url=new URL(window.location.href);url.pathname='/historias';url.search='';url.hash=''
  window.history.replaceState({},'',url)
  notify('Avance guardado en este dispositivo.')
}
...
function publicSiteLabel(){return'mi-mapa.github.io'}
...
function drawTodayPoster(ctx:CanvasRenderingContext2D,chart:Chart,daily:ReturnType<typeof dayReading>){
  ...
  ctx.fillStyle=meta.dark;ctx.font=posterFont(15,800);ctx.fillText('TRES COSAS QUE PUEDES HACER HOY',128,664)
...
function drawSummaryPoster(ctx:CanvasRenderingContext2D,chart:Chart){const meta=elementMeta[chart.dayMaster.element],identity=identityMeta[chart.dayMaster.stem],polarity=chart.dayMaster.polarity==='yin'?'Yin':'Yang';ctx.fillStyle=meta.dark;ctx.textAlign='left';ctx.font=posterFont(18,800);ctx.fillText('MI MAPA EN BREVE',88,178);ctx.font=posterFont(16,700);ctx.fillStyle='rgba(62,56,50,.7)';ctx.fillText('UN RESUMEN DE TUS RESULTADOS',88,210);drawSeal(ctx,790,352,165,meta.color);drawStemPosterIcon(ctx,chart.dayMaster.stem,790,352,210,meta.dark);ctx.fillStyle=meta.dark;ctx.textAlign='left';ctx.font=posterFont(18,800);ctx.fillText('TU CENTRO',88,430);ctx.font=posterFont(fitPosterText(ctx,meta.label+' '+polarity,820,112,64),760,true);ctx.fillText(meta.label+' '+polarity,88,548);ctx.font=posterFont(30,560,true);drawPosterText(ctx,identity.headline,88,630,720,42,3);ctx.fillStyle='rgba(255,252,246,.78)';roundRect(ctx,88,835,904,198,34);ctx.fill();ctx.strokeStyle='rgba(62,56,50,.1)';ctx.stroke();ctx.fillStyle=meta.dark;ctx.font=posterFont(15,800);ctx.fillText('ASÍ SE NOTA EN TI',132,887);ctx.font=posterFont(26,520,true);drawPosterText(ctx,elementPortrait[chart.dayMaster.element],132,932,790,34,3);ctx.fillStyle=meta.color;ctx.beginPath();ctx.arc(916,900,36,0,Math.PI*2);ctx.fill();ctx.fillStyle='#fff';ctx.textAlign='center';ctx.font=posterFont(20,800);ctx.fillText('×',916,907)}
 
function Reading({chart,section,onSection,onHome,onReplay,onTool}:{chart:Chart;section?:ReadingSection;onSection:(section:ReadingSection)=>void;onHome:()=>void;onReplay:()=>void;onTool:(view:View)=>void}){
  ...
  useEffect(()=>{
    ...
    const frame=requestAnimationFrame(()=>document.getElementById(section)?.scrollIntoView({behavior:'smooth',block:'start'}))
    return()=>cancelAnimationFrame(frame)
  },[section])
  const jumpTo=(event:ReactMouseEvent<HTMLAnchorElement>,sectionId:ReadingSection)=>{
    event.preventDefault()
  ...
  return <main className="reading">
    ...
    <div className="readingBody">
      <aside className="readingRail"><p>EXPLORA TU MAPA</p><nav aria-label="Secciones de tu lectura">
        <a href="#perfiles" onClick={event=>jumpTo(event,'perfiles')}><i/>Tus {profileCountLabel(chart)} perfiles<small>Cómo actúas según la situación</small></a>
      ...
      <div className="readingContent">
        ...
        <section className="readingSection" id="elementos"><SectionHead kicker="TU MEZCLA" title="Cinco maneras de responder a la vida."/><div className="elementFeature"><div><small>MÁS DISPONIBLE</small><ElementMark element={strong}/><h3>{elementMeta[strong].label}</h3><p>{elementMeta[strong].sentence}</p></div><div><small>REQUIERE MÁS PRÁCTICA</small><ElementMark element={low}/><h3>{elementMeta[low].label}</h3><p>{elementMeta[low].sentence}</p></div></div><ElementRow chart={chart}/><ShareActions kind="elements" chart={chart} shareLabel="Compartir mi gráfica"/></section>
        <section className="readingSection" id="acciones"><SectionHead kicker="CÓMO ACTÚAS" title="Las respuestas que tienes más a la mano."/><div className="longActions">{topActions(chart).map(([key],index)=><article key={key}><span aria-hidden="true">→</span><div><small>{index===0?'LA USAS CON MAYOR FRECUENCIA':index===1?'TAMBIÉN LA USAS SEGUIDO':'OTRA RESPUESTA FRECUENTE'}</small><h3>{actionMeta[key].name}</h3><p>{actionMeta[key].copy}</p></div></article>)}</div></section>
        <section className="readingSection" id="encuentros"><SectionHead kicker="CHOQUES Y ARMONÍAS" title="Cómo se relacionan tus perfiles."/><div className="interactionGrid">{chart.interactions.length?chart.interactions.map(item=>{const r=interactionReading(item);return <article key={item.id}><small>{item.kind}</small><h3>{r.title}</h3><p>{r.body}</p><Technical>{item.note}</Technical></article>}):<article><h3>Tus perfiles actúan por separado.</h3><p>Cada perfil aparece en situaciones distintas. Algunas fechas pueden hacer que dos de ellos coincidan o entren en conflicto.</p></article>}</div></section>
...
function ExpertChart({chart}:{chart:Chart}){
  ...
  const detail=pick.kind==='void'
    ...
    :pick.kind==='branch'
    ?{title:`${branches[pillar.branch].label}: cómo reaccionas en esta parte de tu vida`,body:branchPace[pillar.branch],technical:`Rama terrestre · ${elementMeta[branches[pillar.branch].element].label}`}
    :pick.kind==='hidden'
      ?{title:`${identityMeta[selectedStem].name}: un comportamiento menos visible`,body:`Esta parte suele notarse después de conocerte mejor. Describe una conducta que usas en ciertas situaciones: ${identityMeta[selectedStem].body.charAt(0).toLowerCase()+identityMeta[selectedStem].body.slice(1)}`,technical:`Componente interno · ${stems[selectedStem].label}`}
  ...
  const explanation=(className:string)=><aside className={`expertExplanation ${className}`} aria-live="polite">{pick.kind==='void'?<span className="voidGlyph">○</span>:pick.kind==='branch'?<AnimalGlyph branch={pillar.branch} size={52}/>:<Glyph stem={selectedStem} size={52}/>}<div><small>{metaFor(pick.pillar).eyebrow} · {pick.kind==='stem'?'perfil principal':pick.kind==='branch'?'animal del pilar':pick.kind==='hidden'?'componentes internos':'aprendizaje personal'}</small><h3>{detail.title}</h3><p>{detail.body}</p><Technical>{detail.technical}</Technical></div></aside>
  ...
  return <section className="readingSection expertSection" id="carta-completa">
    ...
    <div className="expertCarousel"><button type="button" className="expertCarouselArrow expertCarouselPrev" onClick={()=>slide(-1)} aria-label="Ver pilar anterior">←</button><div ref={chartRef} className={`expertChart expertChart-${order.length}`} role="group" aria-label="Tus pilares natales y auxiliares">{order.map(key=>{const item=pillarFor(key),meta=metaFor(key);return <div className="expertPillar" key={key}>{item?<article className={key==='day'?'dayColumn':''}>
      ...
      <div className="hiddenStems"><small>COMPONENTES INTERNOS</small>{item.hidden.map(hidden=><button key={hidden} aria-pressed={pick.pillar===key&&pick.kind==='hidden'&&pick.stem===hidden} onClick={()=>setPick({pillar:key,kind:'hidden',stem:hidden})} style={{'--chip':elementMeta[stems[hidden].element].soft,'--chip-dark':elementMeta[stems[hidden].element].dark} as CSSProperties}><Glyph stem={hidden} size={20}/><span>{identityMeta[hidden].name}</span></button>)}</div>
...
function TemporalStrip({chart,onTool}:{chart:Chart;onTool:(view:View)=>void}){
  ...
  return <section className="readingSection temporalStrip" id="ahora"><SectionHead kicker="TU MOMENTO ACTUAL" title="Cada fecha favorece actividades distintas."/><div className="temporalCards">
...
function PersonalClashNotice({reading,compact=false}:{reading:ReturnType<typeof dayReading>;compact?:boolean}){
  if(!reading.personalClash.active)return null
  return <aside className={`personalClashNotice${compact?' compact':''}`}><AnimalGlyph branch={reading.personalClash.dayBranch} size={compact?48:66}/><div><p className="eyebrow">DÍA DE CHOQUE PERSONAL</p><h2>{reading.personalClash.title}</h2><p>{reading.personalClash.body}</p></div></aside>
}
...
function TodayPage({chart,library,active,onSwitch,onHome,onReading,onTool}:ToolPageProps){
  ...
  return <main className="toolPage"><ToolHeader chart={chart} library={library} active={active} onSwitch={onSwitch} onHome={onHome} onReading={onReading} onTool={onTool}/><ToolTabs current="today" onTool={onTool}/>
    ...
    <section className="dayActions"><article><small>TE RECOMENDAMOS</small><h2>Tres cosas que puedes hacer</h2><ul>{reading.opportunity.map(item=><li key={item}>{item}</li>)}</ul></article><article><small>ES MEJOR DEJAR PARA OTRO DÍA</small><h2>Dos cosas que conviene posponer</h2><ul>{reading.margin.map(item=><li key={item}>{item}</li>)}</ul></article></section>
...
function MonthPage({chart,library,active,onSwitch,onHome,onReading,onTool}:ToolPageProps){
  ...
  return <main className="toolPage"><ToolHeader chart={chart} library={library} active={active} onSwitch={onSwitch} onHome={onHome} onReading={onReading} onTool={onTool}/><ToolTabs current="month" onTool={onTool}/>
    ...
    <section className="monthLayout"><article className="monthFocus"><div><p className="eyebrow">QUÉ CONVIENE ATENDER ESTE MES</p><h2>{reading.area.title}</h2><p>{reading.area.theme}</p></div><ol>{reading.area.actions.map(item=><li key={item}>{item}</li>)}</ol></article><div className="monthDetail"><article><small>PON ATENCIÓN</small><h3>Cuida tu tiempo y tus pendientes</h3><p>{reading.area.care}</p></article><article><small>FECHAS RECOMENDADAS</small><h3>Tres oportunidades del mes</h3><div className="featuredDays">{reading.featured.map(day=><button key={day.date} onClick={()=>openDay(day.date)}><span className="featuredDate"><small>DÍA</small><b>{partsFromKey(day.date).day}</b></span><span>{day.rhythm}</span><small>{day.opportunity[0]}</small></button>)}</div></article></div></section>
  </main>
}
...
function CyclesPage({chart,library,active,onSwitch,onHome,onReading,onTool,onSetSex}:ToolPageProps&{onSetSex:(sex:'female'|'male')=>void}){
  ...
  return <main className="toolPage"><ToolHeader chart={chart} library={library} active={active} onSwitch={onSwitch} onHome={onHome} onReading={onReading} onTool={onTool}/><ToolTabs current="cycles" onTool={onTool}/>
    ...
    <section className="cycleIntro"><p>Tu primer ciclo de diez años comenzó el {formatFullDate(reading.startDate)}, cuando tenías {reading.startAge} años cumplidos. Antes transcurrió tu etapa inicial; después, cada nuevo ciclo empieza a los {reading.startAge+10}, {reading.startAge+20}, {reading.startAge+30} años y dura otros diez años.</p></section>
