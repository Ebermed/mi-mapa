...
function drawSummaryPoster(ctx:CanvasRenderingContext2D,chart:Chart){const meta=elementMeta[chart.dayMaster.element],identity=identityMeta[chart.dayMaster.stem],strong=strongestElement(chart)[0],low=lowestElement(chart)[0];ctx.fillStyle=meta.dark;ctx.textAlign='center';ctx.font=posterFont(16,800);ctx.fillText('MI PERFIL PRINCIPAL',540,155);drawSeal(ctx,540,305,112,meta.color);drawStemPosterIcon(ctx,chart.dayMaster.stem,540,305,138,meta.dark);ctx.font=posterFont(fitPosterText(ctx,identity.name,770,94,60),760,true);ctx.fillText(identity.name,540,500);ctx.font=posterFont(28,500,true);drawPosterText(ctx,identity.headline,540,555,810,37,3,'center');[['MI RECURSO MÁS DISPONIBLE',strong],['EL QUE PUEDO TRABAJAR MÁS',low]].forEach(([label,element],index)=>{const key=element as ElementKey,x=index?552:88;ctx.fillStyle='rgba(255,255,255,.7)';roundRect(ctx,x,690,440,205,30);ctx.fill();drawStemPosterIcon(ctx,elementStem[key],x+74,792,84,elementMeta[key].dark);ctx.fillStyle=meta.dark;ctx.textAlign='left';ctx.font=posterFont(13,800);ctx.fillText(String(label),x+135,746);ctx.font=posterFont(37,700,true);ctx.fillText(elementMeta[key].label,x+135,805);ctx.font=posterFont(16,500);drawPosterText(ctx,elementMeta[key].sentence,x+135,838,260,23,2)});PILLAR_ORDER.forEach((key,index)=>{const p=chart.pillars[key],x=88+index*230;ctx.fillStyle='rgba(255,255,255,.62)';roundRect(ctx,x,935,208,190,24);ctx.fill();drawStemPosterIcon(ctx,p.stem,x+55,998,54,elementMeta[stems[p.stem].element].dark);drawAnimalPosterIcon(ctx,p.branch,x+160,990,38,elementMeta[branches[p.branch].element].dark);ctx.fillStyle=meta.dark;ctx.textAlign='center';ctx.font=posterFont(12,800);ctx.fillText(pillarMeta[key].eyebrow.toUpperCase(),x+104,1058);ctx.font=posterFont(26,700,true);ctx.fillText(identityMeta[p.stem].name,x+104,1094)})}
